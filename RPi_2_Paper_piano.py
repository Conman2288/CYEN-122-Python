...
class Note(pygame.mixer.Sound):
    ...
    def __init__(self, frequency, volume, wave_form_type):
        self.frequency = frequency
        self.wave_form = wave_form_type
...
def play_song():
    ...
    for part in song:
        note, duration = part
        # if it's a silennce, delay for its duration
        if (note == "SILENCE"):
            sleep(duration)
...
# (16 bis signed), channels (mono), and buffer size (1KB)
# then, initialize pygame library
...
# reset the GPIO pins when Ctrl+C is pressed
try:
    while (True):
        ...
        # start a timer
        start = time()
        # play a note when pressed . . . until released and detect play/record
        key = wait_for_note_start()
        ...
        # if he record button was pressed
        if (key == "record"):
